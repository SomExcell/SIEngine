...
Mesh::Mesh(std::vector<Vertex> vertices, std::vector<GLuint> indices, std::vector<TextureTwo> textures):vertices(vertices),indices(indices),textures(textures)
{
    setupMesh();
}
...
void Mesh::draw()
{
    ...
    for(unsigned int i = 0; i < textures.size(); i++)
    {
        ...
        glUniform1i(glGetUniformLocation(Window::objectShader->getID(), (name + number).c_str()), i);
